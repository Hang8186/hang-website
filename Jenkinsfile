...
//   agent { label 'jenkins-agent' }
  ...
  stages {
    stage('Build Docker Image') {
      ...
      post {
        success {
          slackSend (channel: "#project", color: 'good', message: "${env.commitDetails}")
        }  
        failure {
          slackSend (channel: "#project", color: 'danger', message: "FAILURE: Docker image build and push failed.")
        }
      }
    }
    stage('Deploy') {
